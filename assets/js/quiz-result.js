...
let depressionQuestions = [
  // an array of objects depressionQuestions[0].question, depressionQuestions[1].response PHQ-9
  { question: "Are you having thoughts that you would be better off dead, or of hurting yourself?", response: responseType.yesNo },
  { question: "Are you having trouble concentrating on things such as reading the newspaper or watching TV?", response: responseType.yesNo },
  { question: "Are you feeling bad about yourself (feel like a failure or let your family down)", response: responseType.yesNo },
  { question: "Do you have a poor appetite or are you overeating?", response: responseType.yesNo },
  { question: "Are you feeling tired or having little energy?", response: responseType.yesNo },
  { question: "Are you having trouble falling or staying asleep, or sleeping too much?", response: responseType.yesNo },
  { question: "Are you feeling down, depressed, or hopeless?", response: responseType.yesNo },
  { question: "Do you have little interest or pleasure in doing things?", response: responseType.yesNo },
]; // 8 questions
...
let anxietyQuestions = [
  // an array of objects GAD-7
  { question: "Are you feeling nervous, anxious, or on edge?", response: responseType.yesNo },
  { question: "Are you feeling unable to stop or control worrying?", response: responseType.yesNo },
  { question: "Are you worrying too much about different things?", response: responseType.yesNo },
  { question: "Are you having trouble relaxing?", response: responseType.yesNo },
  { question: "Are you so restless that it is hard to sit still?", response: responseType.yesNo },
  { question: "Are you feeling easily annoyed or irritable?", response: responseType.yesNo },
  { question: "Are you feeling as if something awful might happen?", response: responseType.yesNo },
]; // 7 questions

let questionBank = [depressionQuestions, anxietyQuestions, addictionQuestions, impairmentQuestions, schQuestions, ptsdQuestions];

function radioQuestion(questionObj) {
  // takes {question, response}
...
function quizSubmitButtonHandler() {
  ...
  quizSubmitButtonEl.addEventListener("click", function () {
    ...
    if (score / quizLength > 0.5) {
      // simple comparison - if over half questions are marked 'yes', then it's active
      symptoms[0] = "Active";
    } else {
      symptoms[0] = "Inactive";
    }
